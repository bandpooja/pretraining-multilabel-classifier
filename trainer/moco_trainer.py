...
def load_model(model_dir, model_name):
    model = torch.load(os.path.join(model_dir, model_name), weights_only=False)
    return model
...
def train_classifier_w_pretraining(
    ...
    fine_tune_model(model=model, train_loader=train_loader, val_loader=val_loader, 
                    ...
                    model_path=os.path.join(model_dir, f"ft_resnet18_.pth"))
    model = torch.load(os.path.join(model_dir, f"ft_resnet18_.pth"), weights_only=False)
    return model
